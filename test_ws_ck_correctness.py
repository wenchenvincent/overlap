...
def main():
    ...
    print(f"primus CK WS correctness sweep ({len(cases)} cases)")
    all_ok = True
    for G, M, K, N, trans_b in cases:
        ...
        out_ws = pt_grouped_gemm(
            a, b, gs, trans_b=trans_b, work_steal=True, ws_counter=counter)
        ...
        print(
            ...
            f"max_abs={max_abs:.1e} max_rel={max_rel:.1e} "
            f"counter={ctr} NaN={nan_ws} [{status}]"
        )
